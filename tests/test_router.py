...
def _stats():
    return {
        "fast-cheap/m":  {"backend": "ollama",  "avgLatencyMs": 800,  "avgCostUSD": 0.0,  "errorRate": 0.10, "requests": 50},
        "slow-reliable/m": {"backend": "oci_genai", "avgLatencyMs": 6000, "avgCostUSD": 0.02, "errorRate": 0.00, "requests": 80},
        "flaky/m":       {"backend": "openai",  "avgLatencyMs": 1500, "avgCostUSD": 0.05, "errorRate": 0.40, "requests": 20},
    }
...
def test_low_bias_prefers_fast_cheap():
    d = router.choose_model(
        prompt_complexity=0.0, pool=list(_stats()), stats=_stats(),
        health_fn=_health, cost_fn=_cost, bias=0.0,
    )
...
def test_high_bias_prefers_reliable():
    d = router.choose_model(
        prompt_complexity=0.0, pool=list(_stats()), stats=_stats(),
        health_fn=_health, cost_fn=_cost, bias=1.0,
    )
...
def test_complexity_nudges_toward_quality():
    ...
    low = router.choose_model(prompt_complexity=0.0, pool=list(_stats()), stats=_stats(),
                              health_fn=_health, cost_fn=_cost, bias=0.5)
    high = router.choose_model(prompt_complexity=1.0, pool=list(_stats()), stats=_stats(),
                               health_fn=_health, cost_fn=_cost, bias=0.5)
...
def test_flaky_model_penalized_by_error_rate():
    d = router.choose_model(prompt_complexity=0.0, pool=list(_stats()), stats=_stats(),
                            health_fn=_health, cost_fn=_cost, bias=1.0)
...
def test_unhealthy_backend_demoted():
    ...
    d = router.choose_model(prompt_complexity=0.0, pool=list(_stats()), stats=_stats(),
                            health_fn=health, cost_fn=_cost, bias=1.0)
    # the otherwise-reliable model loses its health component
    assert d["model"] != "slow-reliable/m"


def test_no_history_uses_neutral_defaults():
    d = router.choose_model(prompt_complexity=0.0, pool=["new/model"], stats={},
                            health_fn=_health, cost_fn=_cost, bias=0.5,
...
def test_empty_pool_returns_none():
    d = router.choose_model(prompt_complexity=0.5, pool=[], stats={},
                            health_fn=_health, cost_fn=_cost)
