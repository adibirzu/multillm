...
# The OCI SDK is a heavy optional dependency (install with the `oci` extra). The
# adapter degrades gracefully without it; skip these tests when it is absent so
# CI without the extra stays green.
...
def test_extract_response_text_cohere_and_generic():
    ...
    inner = types.SimpleNamespace(
        message=types.SimpleNamespace(
            content=[types.SimpleNamespace(text="generic answer")]
        )
    )
