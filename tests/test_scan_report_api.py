...
def test_scan_report_api_ingests_lists_and_exports_only_current_tenant(
    ...
    created = client.post(
        "/api/scan-reports", json=_payload(), headers={"X-MultiLLM-Tenant": "tenant-a"}
    )
    ...
    assert (
        client.get(
            f"/api/scan-reports/{report_id}", headers={"X-MultiLLM-Tenant": "tenant-b"}
        ).status_code
        == 404
    )

    summary = client.get(
        "/api/scan-reports/summary", headers={"X-MultiLLM-Tenant": "tenant-a"}
    )
    assert summary.json()["data"]["findingsBySeverity"] == {"high": 1}
    exported = client.get(
        "/api/scan-reports/export?format=csv", headers={"X-MultiLLM-Tenant": "tenant-a"}
    )
